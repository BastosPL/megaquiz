...
export const perfilProfissionalDisc: Quiz = {
  ...
  introText: "O modelo DISC é uma das ferramentas de análise comportamental mais utilizadas no mundo corporativo. Criado pelo psicólogo William Moulton Marston em 1928 — o mesmo que inventou o detector de mentiras e criou a Mulher-Maravilha — o DISC mapeia quatro dimensões do comportamento humano: Dominância, Influência, Estabilidade e Conformidade.\n\nA ideia central é simples: pessoas diferentes respondem ao ambiente de formas diferentes. Algumas são orientadas a resultados e decisões rápidas (Dominância). Outras focam em comunicação e motivação do grupo (Influência). Há quem priorize consistência, harmonia e apoio à equipe (Estabilidade). E há quem funcione melhor com dados, processos e padrões de qualidade (Conformidade).\n\nEmpresas como Google, Amazon e Deloitte usam variações do DISC em processos de recrutamento, formação de equipes e desenvolvimento de liderança. Não porque o modelo seja perfeito — nenhum framework comportamental é — mas porque ele oferece uma linguagem comum para discutir estilos de trabalho sem julgamento de valor. Nenhum perfil é melhor que outro; o que importa é como os perfis se complementam dentro de um time.\n\nEste quiz é uma versão simplificada e recreativa do modelo DISC. São 12 perguntas sobre como você se comunica, toma decisões, lida com conflitos e se organiza no trabalho. O resultado indica seu perfil predominante, com sugestões de áreas profissionais e dicas de como seu estilo se relaciona com os outros três.",
